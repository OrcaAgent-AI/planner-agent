...
async def plan_step(
    ...
    plan_model = configurable.get("plan_model", "") or ctx.plan_model
    planner_llm = load_chat_model(plan_model)

    if not planner:
        system_prompt = (
            configurable.get("system_prompt", "") or ctx.system_prompt or SYSTEM_PROMPT
        )
        planner = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    system_prompt.format(system_time=datetime.now(tz=UTC).isoformat()),
                ),
    ...
    plan = await planner.ainvoke(
        {"input": input_text, "system_time": datetime.now(tz=UTC).isoformat()}
    )
    plan_str = "\n".join(f"{step}" for i, step in enumerate(plan.steps))
    return {
        "input": input_text,
        "plan": plan.steps,
...
async def execute_step(
    ...
    """
    global executor
    plan_str = "\n".join(f"{step}" for i, step in enumerate(state.plan))
    task = state.plan[0]
    task_formatted = EXECUTION_PROMPT.format(
        plan_str=plan_str, task=task, past_steps=state.past_steps
    )

    ctx = get_context(runtime)
    configurable = config.get("configurable", {})

    system_prompt = configurable.get("system_prompt", "") or ctx.system_prompt
    system_message = system_prompt.format(system_time=datetime.now(tz=UTC).isoformat())
    execute_model = configurable.get("execute_model", "") or ctx.execute_model
    execute_llm = load_chat_model(execute_model)

    # Load tools asynchronously
    tools = await get_tools(config)
    if not executor:
        executor = create_agent(
            model=execute_llm, tools=tools, system_prompt=system_message
        )
...
async def replan_step(
    ...
    replan_model = configurable.get("replan_model", "") or ctx.replan_model
    replanner_llm = load_chat_model(replan_model)
...
def create_graph(name: str | None = None):
    """Create and compile the planner agent workflow graph."""
    ...
    return workflow.compile(name=name or "Planner Agent")
